# Data19 - Python 데이터 분석 실습 자료

이 저장소는 **Python 데이터 분석 실습**을 위한 샘플 데이터셋과 예제 스크립트를 제공한다.  
`data19` 디렉토리 안 데이터는 교육·학습용으로 구성했다.
...
```
data19/
├── sales_data.csv          # 가상 쇼핑몰 매출 데이터 (2036년)
...
| 파일명 | 형식 | 설명 | 주요 컬럼 |
|--------|------|------|----------|
| `sales_data.csv` | CSV | 2036년 가상 쇼핑몰 일별 매출 | `date`, `product_id`, `quantity`, `revenue`, `customer_id` |
